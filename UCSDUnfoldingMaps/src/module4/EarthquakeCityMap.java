...
public class EarthquakeCityMap extends PApplet {
	...
	private String test1file = "test1.atom";
	private String test2file = "test2.atom";
	private String quiz1file = "quiz1.atom";
	...
	public void setup() {		
		...
		for(Feature city : cities) {
		  cityMarkers.add(new CityMarker(city)); //调用CityMarker.java
		//为什么不能用
		//cityMarkers = MapUtils.createSimpleMarkers(cities); -> 会以默认的格式添加标记
		}
...
//		int len = countryMarkers.size();
//		for (Marker cm : countryMarkers) {
//			int numQuakes = 0;
//			List<PointFeature> earthquakes = ParseFeed.parseEarthquake(this, quiz1file);
//			for (PointFeature eq : earthquakes) {
//				if (isLand(eq)) {
//						if (cm.getStringProperty("name") == eq.getStringProperty("country")) {
//							numQuakes ++;
//						}
//					}
//				else numOceanQuakes ++;
//				}
//				if (numQuakes >0) {
//					System.out.println(cm.getStringProperty("name") + ": " + numQuakes);
//				}
//		}
//			System.out.println("OCEAN QUAKES: " + numOceanQuakes/len);

		// TODO: Explore the efficient method of using HashMap
		//https://www.coursera.org/learn/object-oriented-java/discussions/forums/hVx3hGiEEeauFg5Q7NIZMw/threads/ImopR3xfEeiR-RKlTIHM3g

		HashMap<String, Integer> countryQuakeCount = null;
		
		countryQuakeCount.put("OCEAN QUAKES", 0);
		int numQuakes = 0;
		
		for (Marker cm : countryMarkers) {
			String country;
			country = cm.getProperty("country").toString();
			countryQuakeCount.put(country, 0);
		}
		...
		for (Marker m : quakeMarkers) {
			...
			if (em.isOnLand) {
					...
					if (countryQuakeCount.get(country) != null) {
						numQuakes = countryQuakeCount.get(country) + 1 ;
						countryQuakeCount.put(country, numQuakes);
					}
			}
			else countryQuakeCount.put("OCEAN QUAKES", countryQuakeCount.get("OCEAN QUAKES") + 1 );
		}
			for (String country : countryQuakeCount.keySet()) {
				if (countryQuakeCount.get(country) >0) {
