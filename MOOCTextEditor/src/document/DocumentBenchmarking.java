	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			...
			for (int cnt = 0; cnt < trials; cnt ++) {
				BasicDocument basicText = new BasicDocument (text);
				basicText.getFleschScore();
			}
			long basicEndTime = System.nanoTime();
			double basicEstTime = (basicEndTime - basicStartTime) / 100000000.0;
			System.out.println(basicEstTime + "\t");
			
			long efficientStartTime = System.nanoTime();
			for (int cnt = 0; cnt < trials; cnt ++) {
				EfficientDocument efficientText = new EfficientDocument (text);
				efficientText.getFleschScore();
			}
			long efficientEndTime = System.nanoTime();
			double efficientEstTime = (efficientEndTime - efficientStartTime) / 100000000.0;
